 ...
 */
class Blocklayouts_Plugin_Updater {
	...
	protected function get_license_key() {
		...
		$license = get_option( 'blocklayouts_license_data' );
		if ( ! is_array( $license ) || empty( $license['license_key']['key'] ) ) {
			return null;
		}
		// error_log( print_r( $license['license_key']['key'], true ) );
		return $license['license_key']['key'];
	}
